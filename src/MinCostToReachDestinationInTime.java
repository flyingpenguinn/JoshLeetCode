...
public class MinCostToReachDestinationInTime {
    // for any limited condition of dijkastra: use dist[state] as the dist array where state here is (time, cost)
    // a more costly path may just spend less time so has higher chance of survival
    // same applies to "where the path has at most n stops"
    private int Max = 1000000000;

    public int minCost(int maxTime, int[][] edges, int[] passingFees) {
        // city id, current time, current cost
        ...
        while (!pq.isEmpty()) {
            ...
            Map<Integer, Integer> nm = g[id];
            for (int nk : nm.keySet()) {
                int nt = nm.get(nk);
                int ndist = cost + passingFees[nk];
                int ntime = time + nt;
                if (ntime > maxTime) {
                    continue;
                }
                int cdist = dist[nk][ntime];
                if (cdist > ndist) {
                    dist[nk][ntime] = ndist;
                    pq.offer(new int[]{nk, ntime, ndist});
                }
            }
        }
        return -1;
...
class MinCostToReachDestinationInTimeDp {
    ...
    public int minCost(int maxTime, int[][] es, int[] pt) {
        ...
        for (int j = 0; j <= maxTime; j++) {
            for (int[] e : es) {
                process(pt, dist, j, e, 0, 1, maxTime);
                process(pt, dist, j, e, 1, 0, maxTime);
            }
        }
        int res = Max;
        for (int j = 0; j <= maxTime; j++) {
            res = Math.min(res, dist[n - 1][j]);
        }
        return res >= Max ? -1 : res;
    }

    protected void process(int[] pt, int[][] dist, int j, int[] e, int si, int ti, int maxTime) {
        int from = e[si];
        int to = e[ti];
        int newDist = dist[from][j] + pt[to];
        int newTime = j + e[2];
        if (newTime > maxTime) {
            return;
        }
        dist[to][newTime] = Math.min(dist[to][newTime], newDist);
    }
}
